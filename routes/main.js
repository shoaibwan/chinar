...
router.post("/admin/update-mission", requireAuth, (req, res) => {
  ...
  const { title, subtitle, heading, paragraph1, paragraph2, imagePosition } =
    req.body;
...
router.post("/join", async (req, res) => {
  ...
  try {
    ...
    const transporter = nodemailer.createTransport({
      host: "smtp.gmail.com", // Use the string directly to ensure no .env spaces
      port: 465,
      secure: true, // Port 465 must be true
      auth: {
        user: process.env.SMTP_USER,
        pass: process.env.SMTP_PASS.replace(/\s+/g, ""), // Strips any spaces
      },
      tls: {
        rejectUnauthorized: false, // Prevents local certificate block
        servername: "smtp.gmail.com",
      },
      family: 4, // THIS IS THE PINPOINT FIX: Forces IPv4
      connectionTimeout: 30000, // Increase to 30s
      greetingTimeout: 30000,
      socketTimeout: 30000,
      debug: true, // Enable this to see exactly where it stops in your console
      logger: true,
    });
    ...
    if (error.code === "ESOCKET" || error.code === "ETIMEDOUT") {
      userMessage +=
        "Email service is temporarily unavailable. Your information has been logged and we will contact you soon.";
    } else if (error.code === "EAUTH") {
      userMessage +=
        "Email authentication failed. Your information has been logged and we will contact you soon.";
    } else {
